...
# combat Function; WIP
def combat(player_HP, player_AP):
    fight_yesno = random.randrange(0,2)
    skeleton_HP = 2
    skeleton_AP = 2
    if fight_yesno == 1:
...
# Story Function; WIP
def story(roomnum, visitedroom):
    if roomnum == 1 and visitedroom == False:
        print("You enter the dungeon, readying yourself for whatever lies inside....")
    if roomnum == 4 and visitedroom == False:
        print("You see the dungeon's exit directly ahead, and wonder if you should leave now, or try and gain more riches...")
...
def room1(goldAmount, visited_room):
    ...
    #
    # HINT: Whatever stats for shopping you want to implement should likely exist inside the 'shop' function.
    #
