...
def getHumanInput():
    ...
    while True:
        ...
        try:
            ...
            print("Error: Row must be an integer. Please try again.")
            continue
        if int(row) < 0 or int(row) > gameBoard.GAME_BOARD_HEIGHT-1:
            print("Error; Row must be an integer between 0 and " + str(gameBoard.GAME_BOARD_HEIGHT-1) + ". Please try again.")
            continue
        break
    ...
    while True:
        ...
        try:
            ...
            print("Error: Row must be an integer. Please try again.")
            continue
        if int(col) < 0 or int(col) > gameBoard.GAME_BOARD_HEIGHT-1:
            print("Error; Column must be an integer between 0 and " + str(gameBoard.GAME_BOARD_HEIGHT-1) + ". Please try again.")
            continue
        break
